...
这是一个给 Codex 用的 Google Chrome 会话清理 skill。仓库名称保持 Apple 导向，但脚本现在支持两个 target：
...
- Claude / Anthropic 的网页会话因为旧 cookie 或本地状态反复复用而变得粘滞
...
- macOS Claude target 已在真实 Mac 上完成 `audit -> apply --dry-run -> apply` 实机验证，验证日期为 2026-04-14
...
- Claude 路径已成功创建备份、写入 `12/12` 条 Claude 家族 `session_only` 规则，并删除 Claude 家族 cookie 与可直接识别的 IndexedDB 痕迹
- Chrome 重启后的交互式 Apple / Claude 登录是否完全符合你的账户流，仍需要用户自己再做一次人工确认
...
默认情况下，脚本会按操作系统解析 Chrome 根目录，默认 target 仍然是 `apple`，并从 `Local State` 里读取最近使用的 profile；如果拿不到，就回退到 `Default`。
...
- 备份文件名会带 target，例如 `backup.apple-session-only.*` 或 `backup.claude-session-only.*`。
- `--dry-run` 只展示计划变更，不会真的修改 profile。

## 行为摘要

- 把 target 对应的 cookie 规则写成 Chrome `Preferences` 里的 `session_only` 项。
- 从 `Cookies` SQLite 数据库里删除目标域名族的旧 cookie 行。
- 从 `Preferences` 里删除目标域名族的可直接定位站点状态，例如 `site_engagement`、`media_engagement`。
- 只在文件名里安全编码了目标 host 的情况下，删除可直接识别的 origin storage artifact。
- 这不是永久封禁 Apple、Claude 或 Anthropic；目标是“下次启动更干净”，不是“让登录失效”。
